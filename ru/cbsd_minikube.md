...
В этой статье описывается вариант запуска Minikube с использованием в двух вариантах - использованием ванильного cloud образа Debian и с демонстрацией возможности создания собственных cloud образов CBSD -
мы созданием GOLD образ с целью кеширования и провижена сервиса minikube, что позволит разворачивать minikube за ~10 секунд.

Оба профиля доступны в [базовой поставке/библиотеке профилей](https://github.com/cbsd/cbsd-vmprofiles).

[Minikube](https://minikube.sigs.k8s.io/docs/) - это компактная версия кластера Kubernetes, который удобно запускать на вашем локальном рабочем компьютере в целях тестирования или разработки. 
Например, вы работаете на FreeBSD сервере и вам необходимо запускать некоторые сервисы, ориентированные на запуск в среде Kubernetes. 
Для этого, вы запускаете Minikube в гипервизоре `bhyve`,`XEN` или `QEMU` и работаете с сервисом через тонкий клиент `kubectl`.
...
Помимо TUI и CLI интерфейса, CBSD позволяет писать CBSDfile, которые являются sh скриптом с заранее определенными функциями, поэтому создадим профиль на базе Debian 13 (конечно, вы можете взять любой другой Linux дистрибутив
из коллекции профилей), где в postcreate функции получим и установим <string>.deb</strong> пакет, создадим systemd unit-файл для запуска сервиса и в финале скачаем готовый kubeconfig на наш хостер.
<details>
...
Если вы внимательно смотрели сценарий и результат его работы, то заметите, что мы при каждом создании сервиса скрипты выкачивают .deb пакет с minikube, который
в свою очередь, скачивает необходимые файлы и поды объемом ~3 GB.

Это не страшно, когда мы создаем сервис достаточно редко, но если сервис будет создаваться часто - логично полученный образ диска превратить в GOLD-образа 
и создав один раз, использовать многократно, получив таким образом кеш артефактов. Для этого, минимизируем объем создаваемого образа до объема,
достаточного лишь для первоначальной инициализации - на момент написания этой статьи, 6GB будет достаточно. Также, если вы используете ZFS файловую систему,
нам выгоднее получить на выходе md/vnode файл вместо ZVOL в директории ~cbsd/jails-data/VM/. Для этого, изменим параметры imgsize=, imgtype=, а также удалим каталог 
/var/lib/cloud, чтобы компоненты cloud-init смогли полноценно отработать вновь при запуске образа. Итоговый CBSDfile файл:
...
В этой версии CBSDfile, мы уже безусловно менеджерим маршрут для сети 192.168.49.0/24 на виртуальную машину, поскольку в автоматическом режиме
проверяем доступность сервиса через minikube. Если он отвечает на запрос - мы получаем образ диска который можно использовать как GOLD-образ: ~cbsd/jails-data/minigold-data/dsk1.vhd
...
В предыдушем шаге была продемонстрирован вариант, как можно легким образом создать свой собственный cloud образ на базе существующих образов CBSD. 
Давайте напишем собственный новый CLOUD-профиль для CBSD. Создайте файл ~cbsd/etc/vm-linux-cloud-Minikube-x86_64.conf с таким содержимым:
