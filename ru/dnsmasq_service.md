...
b) Вы можете получить уже готовый образ из репозитория, либо использовать CBSDFile из коллекции рецептов (потребуется установить `git`) для генерации контейнера с нуля. 
